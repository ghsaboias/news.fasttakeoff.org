...
- `worker.ts`: Custom worker extending OpenNext with `scheduled()` and `queue()` handlers.

## Worker Architecture

Next.js runs on Cloudflare Workers via OpenNext (`@opennextjs/cloudflare`). Custom handlers extend the generated worker:

**worker.ts** (project root):
```typescript
import { default as handler } from "./.open-next/worker.js";
import { scheduled, queue } from "./src/lib/cron";

export default {
  fetch: handler.fetch,  // OpenNext handles all Next.js routes
  scheduled,             // Cron triggers (see wrangler.toml)
  queue                  // Finance data processing
};
```

**Configuration** (`wrangler.toml:1`):
```toml
main = "worker.ts"
```
...
- `worker.ts` imports and extends it with scheduled/queue handlers
- Type-safe: uses `worker-configuration.d.ts` (generated by `npx wrangler types`)
- Official pattern: https://opennext.js.org/cloudflare/howtos/custom-worker

## Build, Test, and Development
- `bun run dev`: Next dev server at `localhost:3000`.
- `bun run build`: Next production build.
- `bun run preview:patch:test`: Build OpenNext worker, run `wrangler dev --test-scheduled` (local testing with cron triggers).
