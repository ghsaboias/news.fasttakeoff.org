...
export class InstagramService {
    ...
    async postNews(report: Report): Promise<void> {
        ...
        try {
            ...
            // First ensure the SVG is generated
            console.log(`[INSTAGRAM] Generating SVG for headline: ${report.headline}`);
            const svgResponse = await fetch(svgUrl);
            if (!svgResponse.ok) {
                throw new Error(`Failed to generate SVG: ${svgResponse.status}, ${svgResponse.statusText}`);
            }

            // Then wait for the screenshot to be generated
            console.log('[INSTAGRAM] Generating screenshot from SVG');
            const screenshotResponse = await fetch(screenshotUrl);
            if (!screenshotResponse.ok) {
                throw new Error(`Failed to generate screenshot: ${screenshotResponse.status}, ${screenshotResponse.statusText}`);
            }

            // Ensure the image is fully generated by checking the response
            const contentType = screenshotResponse.headers.get('content-type');
            if (!contentType || !contentType.includes('image')) {
                throw new Error('Screenshot generation did not return an image');
            }

            // Wait for the full response to be available
            await screenshotResponse.blob();

            console.log('[INSTAGRAM] Image generation completed successfully');
